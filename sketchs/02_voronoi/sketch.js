...
const PALETTES = {
    ...
    custom1: ['#f97316', '#facc15', '#4ade80', '#38bdf8'],
    custom2: ['#1f2937', '#10b981', '#f472b6', '#fde68a'],
    custom3: ['#0f172a', '#312e81', '#f59e0b', '#f97316', '#fef3c7']
};
...
const PALETTE_META = {
    ...
    custom1: {label: 'カスタム 1'},
    custom2: {label: 'カスタム 2'},
    custom3: {label: 'カスタム 3'}
};
...
function drawOverlay() {
    ...
    const boxHeight = 138;
    const boxTop = Math.max(18, height - boxHeight - 18);
...
function captureScreenshot() {
    ...
    const safeCount = String(points.length).padStart(2, '0');
    const filename = `voronoi_${safeCount}`;
    screenshotRequest = {filename};
    redraw();
}
...
function initControlPanel() {
    ...
    controlElements.showOverlay = addCheckboxControl(panel, '情報パネルを表示', SETTINGS.showOverlay, (checked) => {
        SETTINGS.showOverlay = checked;
        redraw();
        updateStatusPanel();
    });
    ...
    buttonRow.appendChild(addButton('画像を保存', () => {
        captureScreenshot();
    }));
...
function syncControlPanel() {
    ...
    if (controlElements.showOverlay) {
...
function getPaletteColors(name) {
    const palette = PALETTES[name] ?? PALETTES.giraffe;
    return palette.map((entry) => normalizeColorEntry(entry));
}
